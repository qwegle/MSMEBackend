...
const BankDepositProof = ({ festivalId, onSuccess }) => {
  ...
  const handleSubmit = async () => {
    ...
    formData.append('bankDepositProof', file);
    
    setLoading(true);
    try {
      await claimsApi.uploadBankDepositProof(formData);
      ...
      >
        {file ? (
          ...
          <>
            ...
            <input type="file" accept=".pdf,.jpg,.jpeg,.png" onChange={handleFileChange} className="hidden" id="bank-file" />
            <label htmlFor="bank-file" className="mt-4 inline-block px-4 py-2 bg-gov-blue text-white rounded-lg cursor-pointer hover:bg-gov-blue-dark transition-colors">
              Choose File
            </label>
          </>
        )}
