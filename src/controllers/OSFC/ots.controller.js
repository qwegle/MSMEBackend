...
export const createOTSForm = [
  decryptRequestBody,
  catchAsync(async (req, res, next) => {
    const formData = { ...req.decryptedBody, userId: req.user.id };
    ...
    while (attempt < 10) {
      ...
      try {
        ...
        return sendEncryptedResponse(res, 201, {
          message: 'OTS Form submitted successfully',
          data: newForm,
        });
      } catch (err) {
...
export const getOTSFormById = catchAsync(async (req, res, next) => {
  ...
  sendEncryptedResponse(res, 200, form);
});
...
export const updateOTSForm = [
  ...
  catchAsync(async (req, res, next) => {
    ...
    sendEncryptedResponse(res, 200, {
      message: 'OTS form updated successfully',
      data: updatedForm,
    });
  }),
];

export const ApproveRejectOtsApplication = [
  decryptRequestBody,
  catchAsync(async (req, res, next) => {
    const { otsFormId, flag } = req.decryptedBody;
    ...
    sendEncryptedResponse(res, 200, { message: status_msg, form: updatedForm });
  }),
];

export const approveOtsApplication = [
  decryptRequestBody,
  catchAsync(async (req, res, next) => {
    const { otsFormId } = req.decryptedBody;
    const updatedForm = await OTSForm.findByIdAndUpdate(
      otsFormId,
    ...
    if (!updatedForm) return next(new AppError('OTS application not found', 404));
    sendEncryptedResponse(res, 200, {
      message: 'OTS application approved successfully',
      form: updatedForm,
    });
  }),
];

export const filterOTS = [
  decryptRequestBody,
  catchAsync(async (req, res, next) => {
    ...
    const parsedStatus = cleanedStatus !== undefined && !isNaN(Number(cleanedStatus))
    ...
    if (cleanedUserId) filter.userId = new Types.ObjectId(cleanedUserId);
    ...
    const paginatedData = paginatedRaw.map(ots => ({
      ...ots,
      createdAt: dayjs(ots.createdAt).tz('Asia/Kolkata').format('DD/MM/YYYY'),
    }));
...
export const getOTSStatusCounts = [
  decryptRequestBody,
  catchAsync(async (req, res) => {
    const { branch } = req.decryptedBody;
